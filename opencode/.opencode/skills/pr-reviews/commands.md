...
**IMPORTANT: Prefix all `gh` commands with `GH_HOST=REDACTED` when reviewing PRs on GitHub Enterprise (REDACTED). Do NOT use `--hostname` flag -- it does not exist.**
...
**NOTE:** On enterprise GitHub (REDACTED), `gh pr view` may fail with "Fragment on Bot can't be spread inside RequestedReviewer". Use the REST API fallback:
```bash
GH_HOST=REDACTED gh api repos/{owner}/{repo}/pulls/{number} | jq '{title: .title, body: .body, state: .state, head: .head.ref, base: .base.ref, changed_files: .changed_files, additions: .additions, deletions: .deletions}'
```
...
GH_HOST=REDACTED gh api repos/{owner}/{repo}/pulls/{number} --jq '{title: .title, state: .state, head: .head.ref, base: .base.ref}'
```
