...
### Spotless ktlint Filename Rule
When a file is named for its purpose (e.g., `AutoConfigureMongoDB.kt`) rather than its primary declared type (e.g., `MongoDBExtension`), Spotless may flag a filename mismatch. Suppress with `@file:Suppress("ktlint:standard:filename")` at the top of the file.
